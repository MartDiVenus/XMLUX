...
if [ -f /tmp/xtextus-estensioneLmx ]; then
   ...
   if [ ! -f /usr/share/vim/vim90/syntax/xml.vim ]; then
     sudo cp /usr/local/lib/xmlux/xtextus/lmx/usr/share/vim/vim90/syntax/xml.vim /usr/share/vim/vim90/syntax
     sudo chown root:root /usr/share/vim/vim90/syntax/xml.vim
     sudo chmod ugao-xrw /usr/share/vim/vim90/syntax/xml.vim
     sudo chmod u+w /usr/share/vim/vim90/syntax/xml.vim
     sudo chmod uga+r /usr/share/vim/vim90/syntax/xml.vim
     sudo chmod ug+r /usr/share/vim/vim90/syntax/xml.vim
   fi
...
if [ -f /tmp/xtextus-estensioneXml ]; then
   ...
   if [ ! -f /usr/share/vim/vim90/syntax/xml.vim ]; then
     sudo cp /usr/local/lib/xmlux/xtextus/xml/usr/share/vim/vim90/syntax/xml.vim /usr/share/vim/vim90/syntax
     sudo chown root:root /usr/share/vim/vim90/syntax/xml.vim
     sudo chmod ugao-xrw /usr/share/vim/vim90/syntax/xml.vim
     sudo chmod u+w /usr/share/vim/vim90/syntax/xml.vim
     sudo chmod uga+r /usr/share/vim/vim90/syntax/xml.vim
...
if [ ! -f /usr/share/vim/vim90/colors/mart.vim ]; then
   sudo cp /usr/local/lib/xmlux/xtextus/xml/usr/share/vim/vim90/colors/mart.vim /usr/share/vim/vim90/colors
   sudo chown root:root /usr/share/vim/vim90/colors/mart.vim
   sudo chmod ugao-xrw /usr/share/vim/vim90/colors/mart.vim
   sudo chmod u+w /usr/share/vim/vim90/colors/mart.vim
   sudo chmod uga+r /usr/share/vim/vim90/colors/mart.vim
...
then
	cat /tmp/xtextus-actionRenderedR | sed 's/.*=//g' > /tmp/xtextus-idToRenderR

	idToRenderR="$(cat /tmp/xtextus-idToRenderR)"


	## Invece di intervenire sul codice lmx, lavoro direttament sul xml.
#	grep -n "ID=\"$idToRenderR\"" /tmp/xtextus-css000 | cut -d: -f1,1 > /tmp/xtextus-idToRenderRBeginLine

#nLineBeginIdToRenderR=$(cat /tmp/xtextus-idToRenderRBeginLine)

#grep -n "<!-- end $idToRenderR -->" /tmp/xtextus-css000 | cut -d: -f1,1 > /tmp/xtextus-idToRenderREndLine

	
	grep -n "ID=\"$idToRenderR\"" $targetFile.xml | cut -d: -f1,1 > /tmp/xtextus-idToRenderRBeginLine

nLineBeginIdToRenderR=$(cat /tmp/xtextus-idToRenderRBeginLine)

grep -n "<!-- end $idToRenderR -->" $targetFile.xml | cut -d: -f1,1 > /tmp/xtextus-idToRenderREndLine


####### Se dovessi rimuovere il blocco, invece io devo selezionarlo
nLineEndIdToRenderR=$(cat /tmp/xtextus-idToRenderREndLine)

echo $nLineEndIdToRenderR - $nLineBeginIdToRenderR | bc > /tmp/xtextus-linesToRender

linesToRenderR=$(cat /tmp/xtextus-linesToRender)

linesToRenderRPlusComment=$(($linesToRenderR + 1))


## Rimuovo dalla I riga all'inizio del blocco da selezionare
echo $(($nLineBeginIdToRenderR - 2)) > /tmp/xtextus-realFirstLine
...
grep "$idToRenderR$" $targetFile-lmxv/notKin/$targetFile-notKin_elements-ids.lmxv | awk '$1 > 0 {print $1}' > /tmp/xtextus-relativeElement
...
rm -fr /tmp/renderCss-xmlux

mkdir /tmp/renderCss-xmlux

cp -r . /tmp/renderCss-xmlux/

## in questo modo copio eventuali immagini da caricare

rm /tmp/renderCss-xmlux/*.back

rm /tmp/renderCss-xmlux/*.css
...
# Ora non ho tempo.
rm /tmp/renderCss-xmlux/*.dtd

rm /tmp/renderCss-xmlux/*.xml

rm /tmp/renderCss-xmlux/*.lmx

rm -f /tmp/renderCss-xmlux/*.lmxv

rm -fr /tmp/renderCss-xmlux/*lmxv*

rm /tmp/renderCss-xmlux/*.lmxp

rm -f /tmp/renderCss-xmlux/*.lmxe

cp /tmp/xtextus-bloccoSelezionatoIntestato /tmp/renderCss-xmlux/renderedCss.xml

cp $targetFile.css /tmp/renderCss-xmlux/renderedCss.css



#cd /tmp/renderCss-xmlux
...
then
	chromium /tmp/renderCss-xmlux/renderedCss.xml
...
cp /tmp/renderCss-xmlux/renderedCss.xml $path
cp /tmp/renderCss-xmlux/renderedCss.css $path
sync
...
then
	...
	cat  /tmp/xtextus-actionRenderedR-name | sed 's/.*=//g' > /tmp/xtextus-idToRenderR-name
	nameToRender=$(cat /tmp/xtextus-idToRenderR-name)

	/usr/local/bin/xmluxv -s --find-name="$nameToRender" --f=$targetFile

grep "\"$nameToRender\"" $targetFile-lmxv/kin/$targetFile-ids-names.lmxv | awk '$1 > 0 {print $1}' > /tmp/xtextus-idToRenderR
...
then
	cat /tmp/xtextus-actionRenderedR-title | sed 's/.*=//g' > /tmp/xtextus-idToRenderR-title
	titleToRender=$(cat /tmp/xtextus-idToRenderR-title)

/usr/local/bin/xmluxv -s --find-title="$titleToRender" --f=$targetFile

grep "$titleToRender$" $targetFile-lmxv/kin/$targetFile-ids-titles.lmxv | awk '$1 > 0 {print $1}' > /tmp/xtextus-idToRenderR
