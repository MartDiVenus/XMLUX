...
#			java /usr/local/lib/xmlux/java/xmluxe/add/matterBrief/book/synopsisFileWriter7.java
			...
			###### I sectione: mi posiziono sulla riga $leggoNumberLine ed elimino tutto ciò che 
...
########### II sectione: Mi posiziono sulla I riga ed elimino tutto ciò che 
...
## I if
		...
		## l'elemento da aggiungere è una sectione 
...
### III if
			...
			then
				...
				echo "è la prima sectione del documento" > /tmp/xmluxe-Ichild
...
cat /tmp/xmluxe-IBloccoRadix /tmp/xmluxe-IIBloccoRadix > $targetFile.lmx
			...
			grep -n "<!-- end $leggoIdRoot$leggoIdNumericPiecePreviousSection -->" /tmp/xmluxe-css000 | cut -d: -f1 > /tmp/xmluxe-numberLine

		fi
			leggoNumberLine=$(cat /tmp/xmluxe-numberLine)

###### I section: mi posiziono sulla riga $leggoNumberLine ed elimino tutto ciò che 
##### è al di sotto di essa
echo $leggoNumberLine+1| bc > /tmp/xmluxe-numberLinePlus1
...
$leggoIDToAdd."
			...
			## non è detto che esista una section, non è obbligatoria
#			if [ -f /tmp/xmluxe-elementSection ]; then
...
#			else
			...
			## se non esiste né la sectione, né la sinossi, allora appendo subito dopo l'inizio di <root
...
#			if [ ! -f /tmp/xmluxe-elementSubsection ]; then
			...
			echo "I subsection of a section" > /tmp/xmluxe-firstElementSubsection
...
#	fi
	...
	# La chiusura del paragrafo consiste di 2 linee, la </saxum> e la <!-- end 'Id del paragrafo' -->
#echo $leggoNumberLine -1| bc > /tmp/xmluxe-numberLineMinus1
...
#						else
						...
						## quindi in tal caso non esiste né il capitolo, né la sinossi; la sectione non la testo
						## perché se non esiste il capitolo non esiste nemmeno la sectione.
...
#			fi
			...
			## Non fare confusione tra I child e l'esistenza di ...-elementSubsubsection, il I child si riferisce
			## all'apsectionenza a uno specifico padre, mentre  ...-elementSubsubsection si riferisce all'esistenza
			## di un elemento 'subsubsection' all'interno di tutto il documento -- non all'interno di uno specifico padre --.
#			if [ ! -f /tmp/xmluxe-elementSubsubsection ]; then
				...
				### first subsubsection not of subsection but of the entire document
...
#	fi
	...
	echo "esiste già un elemento subsubsection" > /dev/null
...
## l'elemento da aggiungere è una sottosezione
		...
		## Controllo coerenza locale, se non esiste il padre (element subsubsection)
		...
		then
			...
			echo "No subsubsection element exists to have the child $leggoIDToAdd you would like to add."
...
#			if [ ! -f /tmp/xmluxe-elementParagraph ]; then
				...
				### first paragraph not of a subsubsection but of the entire document
...
################# INIZIO SOTTOSOTTOSEZIONE LMXE, LMX
	...
	## L'elemento da aggiungere è una sottosottosezione
		...
		then
			...
			echo "No paragraph element $leggoIdRoot$idNumericIPiece.$idNumericIIPiece.$idNumericIIIPiece.$idNumericIVPiece exists
...
#			if [ ! -f /tmp/xmluxe-elementSubparagraph ]; then
				...
				### first subparagraph not of a subsubsection but of the entire document
...
echo "Insert title (at line n. 4), and possible content (starting from line n. 5) of the subparagraph to add. 
At the end save and exit.
" >> /tmp/xmluxe-optionW.lmx
...
################# INIZIO PARAGRAPH LMXE, LMX
	...
	then
	# l'elemento da aggiungere è un saxum
		...
		then
			...
			echo "No subparagraph element $leggoIdRoot$idNumericIPiece.$idNumericIIPiece.$idNumericIIIPiece.$idNumericIVPiece.$idNumericVPiece exists
to have the child $leggoIDToAdd you would like to add."
...
#			if [ ! -f /tmp/xmluxe-elementSaxum ]; then

				echo "I saxum of an element" > /tmp/xmluxe-firstElementSaxum

				### first saxum not of an subparagraph but of the entire document

#				clear 

#			java /usr/local/lib/xmlux/java/xmluxe/add/matterBrief/book/saxumFileWriter7.java
...
if [ -f /tmp/xmluxe-optionName.lmx ]; then
	...
	name=$(cat /tmp/xmluxe-insertName)
			...
			echo "Insert title (at line n. 4),
and possible content (starting from line n. 5) of the saxum to add. 
At the end save and exit.
" >> /tmp/xmluxe-optionW.lmx
...
echo "Insert content of the saxum to add, starting from line number 3.
At the end save and exit.
" >> /tmp/xmluxe-optionW.lmx
...
################# INIZIO SUBPARAGRAPH LMXE, LMX
	...
	then
	## l'elemento da aggiungere è un subsaxum
		...
		then
			...
			echo "No saxum element $leggoIdRoot$idNumericIPiece.$idNumericIIPiece.$idNumericIIIPiece.$idNumericIVPiece.$idNumericVPiece.$idNumericVIPiece exists
to have the child $leggoIDToAdd you would like to add."
...
$leggoIDToAdd."
			...
			echo "I subsaxum of an element" > /tmp/xmluxe-firstElementSubsaxum

			

#			if [ ! -f /tmp/xmluxe-elementSubsaxum ]; then

			idNumericPrevious="$idNumericIPiece.$idNumericIIPiece.$idNumericIIIPiece.$idNumericIVPiece.$idNumericVPiece.$idNumericVIPiece"


				### first subsaxum not of a saxum but of the entire document
#				clear 

#			java /usr/local/lib/xmlux/java/xmluxe/add/matterBrief/book/subsaxumFileWriter7.java
...
if [ -f /tmp/xmluxe-optionName.lmx ]; then
	...
	name=$(cat /tmp/xmluxe-insertName)
			...
			echo "Insert title (at line n. 4),
and possible content (starting from line n. 5) of the subsaxum to add. 
At the end save and exit.
" >> /tmp/xmluxe-optionW.lmx
...
echo "Insert content of the subsaxum to add, starting from line number 3.
At the end save and exit.
" >> /tmp/xmluxe-optionW.lmx
