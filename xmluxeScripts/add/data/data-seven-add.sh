...
leggoIdRoot="$(cat /tmp/xmluxe-idRoot)"
	...
	grep "^-st$" /tmp/xmluxePseudoOptions/$a > /tmp/xmluxeTargetFileOp
...
## Quando avrò tempo, devo implementare il codice rendendo operativa l'opzione -st
# anche per gli altri elementi. Di seguito -- da for option ... al primo fi --
# il codice per l'opzione -st solo per gradusI. Ricordati che a riga 481 hai il done
# relativo al suddetto for.
# Non è difficile estendere -st anche agli altri elementi, dovrei replicare il 
# suddetto codice anche nelle restanti sezioni di tale file, selezionando gradusIIStructure.
# Ma potrebbe non avere molto senso, e cmq non ho tempo.
#
for option in {1}

do

if [ -f /tmp/xmluxe-optionSt.lmx ]; then
...
## Invece di usare grep -A, uso un metodo più elegante
...
cat /tmp/xmluxe-pre0 | sed 's/'$gradusIStructure'/'$leggoIDToAdd'/g' | sed '/<\/radix>/d' >> /tmp/xmluxe-pre000

gvim -f /tmp/xmluxe-pre000

echo "" > /tmp/xmluxe-pre0000

cat /tmp/xmluxe-pre0000 /tmp/xmluxe-pre000 >> /tmp/xmluxe-pre
