...
grep " gradusI " $targetFile-lmxv/kin/$targetFile-ids-elements-names.lmxv > /tmp/xmluxse-ID-gradusI-names

### seleziono solo i names
cat /tmp/xmluxse-ID-gradusI-names | sed 's/.* "//g' | sed 's/"//g' > /tmp/xmluxse-onlyNames


translateA () {

### ordino in forma ascendente
cat /tmp/xmluxse-onlyNames | sort > /tmp/xmluxse-onlyNamesSorted

### individuo il numero di righe perché devo ritrovare i corrispondenti id in base
# al nuovo ordine, e muovere
awk '{ nlines++;  print nlines }' /tmp/xmluxse-onlyNames | tail -n1 > /tmp/xmluxse-frequGradusI
...
#mkdir /tmp/xmluxse-nameOrdinati
...
cat /tmp/xmluxse-onlyNamesSorted | head -n$var | tail -n1 > /tmp/xmluxse-fromNameToId

name=$(cat /tmp/xmluxse-fromNameToId)

## Cercando il primo name ordinato, ottengo il suo corrispondente passato ID
grep "\"$name\"" /tmp/xmluxse-ID-gradusI-names | awk '$1 > 0 {print $1}' > /tmp/xmluxse-idOriginali/xmluxse-originalID-$var
...
#echo "\"$name\"" > /tmp/xmluxse-nameOrdinati/xmluxse-nameOrdinato-$var
...
### ordino in forma ascendente
cat /tmp/xmluxse-onlyNames | sort -r > /tmp/xmluxse-onlyNamesSorted
