...
sorta () {
	...
	/usr/local/bin/xmluxv -it -s --f=$targetFile
...
#	read -p "testing 71" EnterNull
	...
	awk '{ nlines++;  print nlines }' /tmp/xmluxsev-blockToViewScelto | tail -n1 > /tmp/xmluxseSort-titlesLines

	nTitlesLines=$(cat /tmp/xmluxseSort-titlesLines)

	declare -i var=0

	for a in `seq $nTitlesLines`

	do
		var=var+1

		cat /tmp/xmluxsev-blockToViewScelto | head -n$var | tail -n1 | awk '{for(i=2;i<=NF;i++) print " " $i}' | tr -d '\n' | sed 's/^.//g' >> /tmp/xmluxse-titleVar00
		echo "" >> /tmp/xmluxse-titleVar00


	done

	sed '/^$/d' /tmp/xmluxse-titleVar00 > /tmp/xmluxse-titleVar

	#read -p "
	#testing 101
	#cat /tmp/xmluxse-titleVar
	#" EnterNull

	cat /tmp/xmluxse-titleVar  | sed 's/.* "//g' | sed 's/"//g' > /tmp/xmluxse-titleVarClean
	
	cat /tmp/xmluxse-titleVarClean | sort  > /tmp/xmluxseSort-titles
	...
	do
		...
		cat /tmp/xmluxseSort-titles  | head -n$var | tail -n1 > /tmp/xmluxseSort-titleVar

		selectedTitle="$(cat /tmp/xmluxseSort-titleVar)"
...
#		" EnterNull
	...
	#	grep -n "^$selectedTitle$" /tmp/xmluxse-titleVar | cut -d: -f1,1 | head -n$var | tail -n1 > /tmp/xmluxseSort-nLineIDVar
	## Ok li ordina bene
## | head -n$var | tail -n1 servono solo se ci sono titoli uguali, ma non devono esserci titoli uguali.

		grep -n "^$selectedTitle$" /tmp/xmluxse-titleVarClean | cut -d: -f1,1 > /tmp/xmluxseSort-nLineIDVar
...
#	read -p "testing 151" EnterNull
	...
	cat /tmp/xmluxse-titleVarClean | sort -r > /tmp/xmluxseReverseSort-titles
	...
	do
		...
		cat /tmp/xmluxseReverseSort-titles  | head -n$var | tail -n1 > /tmp/xmluxseReverseSort-titleVar


		selectedTitle="$(cat /tmp/xmluxseReverseSort-titleVar)"
...
#		nTitlesLinesPlus=$(($nTitlesLines + 1))

#		decremento=$(($nTitlesLinesPlus - $var))

#		grep -n "^$selectedTitle$" /tmp/xmluxse-titleVar | cut -d: -f1,1 | head -n$decremento | tail -n1 > /tmp/xmluxseSort-nLineIDVar
...
#nTitlesLines = $nTitlesLines

#var = $var

#decremento=$(($nTitlesLines - $var))
...
nTitlesLinesPlus=$(echo $nTitlesLines + 1 | bc)


## Devo iniziare da sotto	
declare -i var=$nTitlesLinesPlus

## modifica 21 settembre 2024 seq nTitlesLinesPlus -> nTitlesLines
	for b in `seq $nTitlesLines`
...
# i nomi e cognomi.
	...
	/usr/local/lib/xmlux/xmluxeScripts/sorting/sorting-byTitle-gradusI.sh
...
#vim /tmp/xmluxse-translate-blocco01
#vim /tmp/xmluxse-translate-blocco02
#vim /tmp/xmluxse-translate-blocco03
#vim /tmp/xmluxse-translate-blocco04
#" EnterNull
...
#vim /tmp/xmluxse-translate-blocco03
#vim /tmp/xmluxse-translate-blocco04
#vim /tmp/xmluxse-translate-blocco04New
#" EnterNull
