...
" Last Changed:	Nov 03, 2019, Mario Fantini: Apr 27, 2024
" Filenames:	*.xml, *.lmx
" Last Change:
...
" 20240427 - Mario Fantini: Highlighting xmlux commands, groups, options.
...
" Mario:
" ok: per selezionare blocchi. e.g. \start{
	" bla bla bla
	" }
"syn region  xmlCommand
"	\ start=+\\+
"	\ end=+}+
"	\ display

" ok: seleziona i comandi per i caratteri \c*lux
syn region  xmluxCharCommand
	\ start=+\\c+
	\ end=+[ ^,.;/!?<>{}()"'`\[\]\-\_:!\=]\@=+
	\ display

" ok: seleziona i comandi per i pattern \c*lux
syn region  xmluxPatternCommand
	\ start=+\\p+
	\ end=+[ ^,.;/!?<>{}()"'`\[\]\-\_:!\=]\@=+
	\ display


" ok: seleziona i comandi per iniziare i gruppi \start{gruppo}, \finish{gruppo}
syn region  xmluxGroup
	\ start=+\\start{+
	\ start=+\\finish{+
	\ end=+}+
	\ display

" ok: seleziona le opzioni, e.g. \start{wrap}[width=80]
syn region  xmluxOption
	\ start=+\[+
	\ end=+\]+
	\ display

syn region xmluxDelimiter
	\ start=+{+
	\ end=+}+
	\ display
...
" Mario: char command
hi def link xmluxCharCommand	CharCommand

" Mario: pattern command
hi def link xmluxPatternCommand	PatternCommand

" Mario: group
hi def link xmluxGroup		Group

" Mario: group
hi def link xmluxOption		Option

" Mario: group
hi def link xmluxDelimiter	Delimiter

let b:current_syntax = "lmx"
