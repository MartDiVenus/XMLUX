...
mkdir $percorsoIsolato/$nomeFile-xmlux
		...
		cat /usr/local/lib/xmlux/documentClasses/matter/matter-book/css2/matter-book.lmx > $percorsoIsolato/$nomeFile-xmlux/$nomeFile.lmx
		...
		vim -c ":%s/matter-book.css/$nomeFile.css/g"	$percorsoIsolato/$nomeFile-xmlux/$nomeFile.lmx -c :w -c :q

                vim -c ":%s/matter-book.dtd/$nomeFile.dtd/g"	$percorsoIsolato/$nomeFile-xmlux/$nomeFile.lmx -c :w -c :q
...
mkdir $nomeFile-xmlux
		...
		cat /usr/local/lib/xmlux/documentClasses/matter/matter-book/css2/matter-book.lmx > $nomeFile-xmlux/$nomeFile.lmx
		...
		vim -c ":%s/matter-book.css/$nomeFile.css/g" $nomeFile-xmlux/$nomeFile.lmx -c :w -c :q

                vim -c ":%s/matter-book.dtd/$nomeFile.dtd/g" $nomeFile-xmlux/$nomeFile.lmx -c :w -c :q
