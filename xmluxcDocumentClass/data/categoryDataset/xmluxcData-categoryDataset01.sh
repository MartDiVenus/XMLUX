...
################ INIZIO ROOT = TITLE = TITULUS = CATEGORY-DATASET CSS, DTD
...
do
	...
	if test $leggoBytesMarginTrue -gt 0
		...
		echo "/* 'Series' = $leggoItem  $leggoID */
$leggoItem { display: block; 
font-size: 23pt; font-weight: bold; font-family: Roboto, \"Roboto\", sans-serif; 
margin-left: 270px; margin-right: 300px; text-align: left; padding-top: 2ex; border-left: 2px solid rgb(25, 25, 50);
border-right: 2px solid rgb(25, 25, 50); border-top: 2px solid rgb(25, 25, 50);  border-bottom: 2px solid rgb(25, 25, 50); width: 11.15in }" >> $targetFile.css
...
$leggoItem { display: block; 
font-size: 23pt; font-weight: bold; font-family: Roboto, \"Roboto\", sans-serif; 
margin-left: 270px; margin-right: 300px; text-align: left; padding-top: 2ex }" >> $targetFile.css
...
do
	...
	if test $leggoBytesMarginTrue -gt 0
		...
		echo "/** 'Item' = $leggoItem $leggoID*/
$leggoItem { display: block; font-size: 17pt; font-weight: bold; font-family: Roboto, \"Roboto\", sans-serif; 
padding-top: 2ex; padding-bottom: 4ex; padding-left: 2ex; padding-right: 2ex; border-bottom: 2px solid rgb(25, 25, 50);
width: 10.65in }" >> $targetFile.css

		else ## del margin=true

		echo "/** 'Item' = $leggoItem $leggoID*/
$leggoItem { display: block; font-size: 17pt; font-weight: bold; font-family: Roboto, \"Roboto\", sans-serif; 
padding-top: 3ex; padding-bottom: 1ex; padding-left: 2ex; padding-right: 2ex; border-bottom: 2px solid rgb(25, 25, 50); width: 10.65in }" >> $targetFile.css
...
################ INIZIO KEY or VALUE -> CSS

leggoIdRoot="$(cat /tmp/xmluxc-idRoot)"

grep "ID=\"$leggoIdRoot" /tmp/xmluxc-css05 > /tmp/xmlux-keyOrValueAnd

rm -fr /tmp/xmluxc-css05Split

mkdir /tmp/xmluxc-css05Split

split -l1 /tmp/xmlux-keyOrValueAnd  /tmp/xmluxc-css05Split/
...
echo "/* Key or Value $leggoItem $leggoID */
$leggoItem { display: block; font-size: 95%; font-weight: bold; font-family: Roboto, \"Roboto\", sans-serif;
padding-top: 1ex; padding-bottom: 2ex; border-top: 2px solid rgb(25, 25, 50); width: 10.65in }" >> $targetFile.css
...
do
	...
	## a differenza della sinossi e di Series, posso avere più capitoli in un file *.lmx.	
...
## e.g. tale elemento Series può contenere uno o più sotto-elementi <Key or Value>, dovrei scrivere:
## Item+
##
## <*> per esprimere <zero or more>
## e.g. tale elemento Series può contenere zero o più sotto-elementi <Item>, dovrei scrivere:
## Item*
##
echo "<!-- All that is under \"Series\" = $leggoItem = $leggoID -->
<!ELEMENT $leggoItem ($leggoPartElements)>
<!ATTLIST $leggoItem ID CDATA \"$leggoID\">" > /tmp/xmluxc-preDtd

else

echo "<!-- All that is in \"Series\" = $leggoItem = $leggoID -->
...
################ FINE SERIES CSS, DTD, LOG, XML SCHEMA, XSLT


############## INIZIO ITEM LOG, DTD (b), XSLT, XML SCHEMA
...
### Se non volessi riportare esattamente tutti i sotto-elementi, potrei usare:
## <+> per esprimere <one or more> 
## e.g. tale elemento Item può contenere uno o più sotto-elementi <Key or Value>, dovrei scrivere:
## Key or Value+
##
## <*> per esprimere <zero or more>
## e.g. tale elemento Item può contenere zero o più sotto-elementi <Key or Value>, dovrei scrivere:
## Key or Value*
##
echo "<!-- All that is under \"Item\" = $leggoItem = $leggoID -->
<!ELEMENT $leggoItem ($leggoPcdataList)>
<!ATTLIST $leggoItem ID CDATA \"$leggoID\">" > /tmp/xmluxc-preDtd

else

echo "<!-- All that is in \"Item\" = $leggoItem = $leggoID -->
<!ELEMENT $leggoItem (#PCDATA)>
<!ATTLIST $leggoItem ID CDATA \"$leggoID\">" > /tmp/xmluxc-preDtd
fi
...
################ FINE ITEM LOG, DTD (b), XSLT, XML SCHEMA

#### ok 12 ottobre 10:23 fare anche Series prima, e poi tutto ciò che viene dopo Item

################ INIZIO KEY or VALUE -> DTD, LOGS, XSLT, XML SCHEMA
rm -f /tmp/xmluxc-ElementDtd
...
### Se non volessi riportare esattamente tutti i sotto-elementi, potrei usare:
## <+> per esprimere <one or more> 
## e.g. tale elemento Key or Value può contenere uno o più sotto-elementi <Key or Value>, dovrei scrivere:
## Key or Value+
##
## <*> per esprimere <zero or more>
## e.g. tale elemento Key or Value può contenere zero o più sotto-elementi <Key or Value>, dovrei scrivere:
## Key or Value*
##
echo "<!-- All that is under \"Key or Value\" = $leggoItem = $leggoID -->
<!ELEMENT $leggoItem ($leggoPcdataList)>
<!ATTLIST $leggoItem ID CDATA \"$leggoID\">" > /tmp/xmluxc-preDtd

else

echo "<!-- All that is in \"Key or Value\" = $leggoItem = $leggoID -->
<!ELEMENT $leggoItem (#PCDATA)>
<!ATTLIST $leggoItem ID CDATA \"$leggoID\">" > /tmp/xmluxc-preDtd
...
## dopo <p> dagli elementi che vanno daa Key or Value a gradusVII
